...
function CustomerProfile({ id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // );
        const vehiclesResponse = await VehicleRequest.getVehicleByCustomerHashId(id);
  ...
  useEffect(() => {
    if (singleCustomer && singleCustomer.customer_hash) {
      const fetchOrders = async () => {
        try {
          const orderResponse = await orderService.getOrderByCustomerHash(
  ...
  const handleSubmit = (e) => {
    ...
    const newVehicle = VehicleRequest.addVehicle(vehicleInfo, token);
    newVehicle
    ...
    <div>
      ...
      {vehicle && (
        <div className="icon-rightData">
          ...
          <div className="rightData">
            <h2>Vehicle of {singleCustomer?.customer_first_name}</h2>
      ...
      {customerOrder && (
        <div className="icon-rightData">
          ...
          <div className="rightData">
            <h2>Order of {singleCustomer?.customer_first_name}</h2>
            <Table striped bordered hover>
              <thead>
                <tr>
                  <th>Order ID</th>
